...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdlie, int iCmdShow)
{
	...
	// Create Window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Three Rotating Lights On Sphere : Nandlal Lambole"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, Width, Height, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Initialize(void)
{
	...
	//Light 
	glLightfv(GL_LIGHT0, GL_AMBIENT, LightAmbient_Zero); //f - float. v- Vector
	...
	glLightfv(GL_LIGHT1, GL_AMBIENT, LightAmbient_One); //f - float. v- Vector
	...
	glLightfv(GL_LIGHT2, GL_AMBIENT, LightAmbient_Two); //f - float. v- Vector
...
void Display(void)
{
	...
	glTranslatef(0.0f, 0.0f, -0.1f); //Hybrid Translation with glLookAt()
	Sphere();
	Update();
	glPopMatrix();
