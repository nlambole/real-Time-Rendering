...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdlie, int iCmdShow)
{
	...
	// Create Window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Circle + Square + Triangle + Circle: Nandlal Lambole"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, Width, Height, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
...
void Graph(void)
{
	...
	for (i = 0; i < 4; i += 0.05)
	{
		...
		glVertex3f(-5.0f, Y + i, 0.0f);
		glVertex3f(5.0f, Y + i, 0.0f);
		glEnd();

		glBegin(GL_LINES); //Verticle
		glColor3f(0.0f, 0.0f, 1.0f);
		glVertex3f(X + i, -2.0f, 0.0f);
		glVertex3f(X + i, 2.0f, 0.0f);
		glEnd();
	}
